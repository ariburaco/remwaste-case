...
export async function getSkipsByLocation(
  ...
  try {
    ...
    const data: RealSkip[] = await response.json();
    
    // Transform the data to match our Skip interface
    const transformedData: Skip[] = data.map((skip) => ({
      id: skip.id.toString(),
      ...
      price: {
        amount: skip.price_before_vat * (1 + skip.vat / 100),
        currency: "GBP"
      },
      suitableFor: getSkipSuitableFor(skip.size, skip.allows_heavy_waste)
    }));
    
    return {
      skips: transformedData,
      location: {
...
export async function searchAddress(query: string): Promise<AddressResponse> {
  const url = `https://services.postcodeanywhere.co.uk/Capture/Interactive/Find/v1.00/json3ex.ws?Key=JP84-GY97-YZ96-GG45&Origin=GBR&Countries=GB&Limit=7&Language=en&Text=${encodeURIComponent(query)}`;
...
function getSkipDimensions(size: number): { length: number; width: number; height: number; unit: string } {
...
function getSkipSuitableFor(size: number, allowsHeavyWaste: boolean): string[] {
  ...
  if (size <= 4) {
    ...
    return [...baseItems, "Home renovations", "Garden clearances", "Office clearouts"];
  } else if (size <= 12) {
    return [...baseItems, "Large renovations", "Construction waste", "Retail clearances"];
  } else {
    const items = ["Large commercial clearances", "Factory clearances", "Development projects"];
